...
def delayed_typing(web_element, text):
    ...
    for char in text:
        if random.randint(1,10)  == 5:
            web_element.send_keys(random_ascii[random.randint(0, len(random_ascii)-1)])
            sleep(random.randint(0,5) / 5)
            web_element.send_keys(Keys.BACKSPACE)
        web_element.send_keys(char)
        sleep(round( random.randint(1,5) / random.randint(25, 50), random.randint(0,50)))
...
def create_driver(settings=[]):
    ...
    driver = webdriver.chrome.webdriver.WebDriver(options=options, executable_path = DRIVER_PATH)
