...
def flip():
    response = request(
        API_ENDPOINT, method, apiKey=API_KEY, n=1, min=1, max=100, request_id=""
    )
...
def multi_flip(flip_count):
    ...
    return (
        f'Flips: {flip_count:}\tHeads: {outcomes["heads"]}\tTails: {outcomes["tails"]}'
    )
...
parser.add_argument(
    "-gt", "--greater_than", help="Lower bound for random number.", type=int
)
parser.add_argument(
    "-lt", "--less_than", help="Upper bound for random number", type=int
)
parser.add_argument("-mf", "--multi_flip", help="Number of times to flip", type=int)
parser.add_argument(
    "-rs", "--random_set", help="Generate a set of random numbers.", type=int
)
