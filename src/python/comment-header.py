...
# Default Config 
LINE_LENGTH = 100        
GROWTH_CONSTRAINT = lambda x : floor(x - .30 * x)
GROWTH_FACTOR = 1.05
...
parser.add_argument("-o", "--opening", help = "Opening Comment Token. For example: /* , #, ''' etc.", type = str)
parser.add_argument("-c", "--closing", help = "Closing Comment Token. For example: */, #, ''' etc.", type = str)
parser.add_argument("-s", "--style",   help = "Comment Style Character. For example: *, |, #, = etc.", type = str)
parser.add_argument("-d", "--display", help = "Set flag to see comment copied to clipboard in command line.", action="store_true")
parser.add_argument("-nd", "--no_display", help = "Set flag to hide all messages in command line.", action="store_true")
