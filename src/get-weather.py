...
def validate_args(args):
    if args.country_code:
        ...
        if code_len > 2 or code_len == 0:
            raise ValueError("Country code must be two chracters long! e.g. US, UK etc.")
    return True
...
parser.add_argument('-i', '--imperial', help='Imperial System of measurement. Metric is default', action='store_true')
args = parser.parse_args()

validate_args(args)
units = 'imperial' if args.imperial else config('WEATHER_UNITS')
country_code = args.country_code if args.country_code else config('WEATHER_COUNTRY_CODE')
